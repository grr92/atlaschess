...
export class Ne extends Piece {
    public hasLostPromotion: boolean = false;

    constructor(id: string, color: PieceColor, position: Position) {
        super(id, color, position, 'Ne');
    }

    override clone(): Ne {
        const cloned = new Ne(this.id, this.color, { ...this.position });
        cloned.hasMoved = this.hasMoved;
        cloned.hasLostPromotion = this.hasLostPromotion;
        return cloned;
    }

    getPossibleMoves(board: Board): Position[] {
        const moves: Position[] = [];
        const forwardY = (this.color === 'red' || this.color === 'white') ? -1 : 1;
