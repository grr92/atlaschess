import { useGameStore } from '../store/useGameStore';
import { VariantRegistry } from '../core/variants/variantRegistry';
import type { AppLanguage } from '../store/types';
import type { Translations, VariantMetaI18n, VariantCodexI18n } from './types';

import { en } from './locales/en';
import { es } from './locales/es';
import { ca } from './locales/ca';

import { variantMeta_en, variantCodex_en } from './variants/en';
import { variantMeta_es, variantCodex_es } from './variants/es';
import { variantMeta_ca, variantCodex_ca } from './variants/ca';

const translationsMap: Record<AppLanguage, Translations> = {
    en,
    es,
    ca,
};

const variantMetaMap: Record<AppLanguage, Record<string, VariantMetaI18n>> = {
    en: variantMeta_en,
    es: variantMeta_es,
    ca: variantMeta_ca,
};

const variantCodexMap: Record<AppLanguage, Record<string, VariantCodexI18n>> = {
    en: variantCodex_en,
    es: variantCodex_es,
    ca: variantCodex_ca,
};

const fallbackVariantCodex: VariantCodexI18n = {
    name: 'Unknown Variant',
    rules: {
        intro: 'Rules for this variant have not been documented yet.',
        bullets: [],
        proTip: 'Play carefully.',
    },
    history: {
        intro: 'History for this variant is lost to time.',
        leftBoxTitle: 'Unknown Origins',
        leftBoxDesc: 'Documentation pending.',
        rightBoxTitle: 'Future Updates',
        rightBoxDesc: 'Documentation pending.',
    },
};

const pieceNamesMap: Record<AppLanguage, Record<string, string>> = {
    en: {
        King: 'King',
        Queen: 'Queen',
        Rook: 'Rook',
        Bishop: 'Bishop',
        Knight: 'Knight',
        Pawn: 'Pawn',
        GrantKing: 'King',
        Aanca: 'Aanca',
        Unicorn: 'Unicorn',
        Lion: 'Lion',
        Crocodile: 'Crocodile',
        Giraffe: 'Giraffe',
        Grantpawn: 'Pawn',
        ChaturajiKing: 'King',
        ChaturajiElephant: 'Elephant',
        ChaturajiHorse: 'Horse',
        ChaturajiBoat: 'Boat',
        ChaturajiPawn: 'Pawn',
        FourSeasonsKing: 'King',
        FourSeasonsGeneral: 'General',
        FourSeasonsRook: 'Rook',
        FourSeasonsKnight: 'Knight',
        FourSeasonsBishop: 'Bishop (Alfil)',
        FourSeasonsPawn: 'Pawn',
        General: 'General',
        Alferza: 'Alferza',
        Alfil: 'Alfil',
        Elephant: 'Elephant',
        Horse: 'Horse',
        Boat: 'Boat',
        XiangqiGeneral: 'General',
        XiangqiAdvisor: 'Advisor',
        XiangqiElephant: 'Elephant',
        XiangqiHorse: 'Horse',
        XiangqiChariot: 'Chariot',
        XiangqiCannon: 'Cannon',
        XiangqiSoldier: 'Soldier',
        Khun: 'Lord (Khun)',
        Met: 'Seed (Met)',
        Khon: 'Nobleman (Khon)',
        Ma: 'Horse (Ma)',
        Ruea: 'Boat (Ruea)',
        Bia: 'Cowrie (Bia)',
        Biangai: 'Promoted Cowrie (Biangai)',
        ShogiKing: 'King (Gyoku)',
        ShogiRook: 'Rook (Hisha)',
        ShogiBishop: 'Bishop (Kaku)',
        ShogiGold: 'Gold General (Kin)',
        ShogiSilver: 'Silver General (Gin)',
        ShogiKnight: 'Knight (Keima)',
        ShogiLance: 'Lance (Kyosha)',
        ShogiPawn: 'Pawn (Fuhyo)',
        ShogiDragon: 'Promoted Rook / Dragon (Ryu)',
        ShogiHorse: 'Promoted Bishop / Horse (Uma)',
        ShogiPromotedSilver: 'Promoted Silver (Narigin)',
        ShogiPromotedKnight: 'Promoted Knight (Narikei)',
        ShogiPromotedLance: 'Promoted Lance (Narikyo)',
        ShogiTokin: 'Tokin (Promoted Pawn)',
    },
    es: {
        King: 'Rey',
        Queen: 'Dama',
        Rook: 'Torre',
        Bishop: 'Alfil',
        Knight: 'Caballo',
        Pawn: 'Peón',
        GrantKing: 'Rey',
        Aanca: 'Aanca',
        Unicorn: 'Unicornio',
        Lion: 'León',
        Crocodile: 'Cocodrilo',
        Giraffe: 'Jirafa',
        Grantpawn: 'Peón',
        ChaturajiKing: 'Rey',
        ChaturajiElephant: 'Elefante',
        ChaturajiHorse: 'Caballo',
        ChaturajiBoat: 'Barco',
        ChaturajiPawn: 'Peón',
        FourSeasonsKing: 'Rey',
        FourSeasonsGeneral: 'General',
        FourSeasonsRook: 'Torre',
        FourSeasonsKnight: 'Caballo',
        FourSeasonsBishop: 'Alfil',
        FourSeasonsPawn: 'Peón',
        General: 'General',
        Alferza: 'Alferza',
        Alfil: 'Alfil',
        Elephant: 'Elefante',
        Horse: 'Caballo',
        Boat: 'Barco',
        XiangqiGeneral: 'General',
        XiangqiAdvisor: 'Consejero',
        XiangqiElephant: 'Elefante',
        XiangqiHorse: 'Caballo',
        XiangqiChariot: 'Carro',
        XiangqiCannon: 'Cañón',
        XiangqiSoldier: 'Soldado',
        Khun: 'Señor (Khun)',
        Met: 'Semilla (Met)',
        Khon: 'Noble (Khon)',
        Ma: 'Caballo (Ma)',
        Ruea: 'Barco (Ruea)',
        Bia: 'Cauri (Bia)',
        Biangai: 'Cauri promovido (Biangai)',
        ShogiKing: 'Rey (Gyoku)',
        ShogiRook: 'Torre (Hisha)',
        ShogiBishop: 'Alfil (Kaku)',
        ShogiGold: 'General de Oro (Kin)',
        ShogiSilver: 'General de Plata (Gin)',
        ShogiKnight: 'Caballo (Keima)',
        ShogiLance: 'Lancero (Kyosha)',
        ShogiPawn: 'Peón (Fuhyo)',
        ShogiDragon: 'Dragón (Ryu)',
        ShogiHorse: 'Caballo Dragón (Uma)',
        ShogiPromotedSilver: 'Plata Promovida (Narigin)',
        ShogiPromotedKnight: 'Caballo Promovido (Narikei)',
        ShogiPromotedLance: 'Lanza Promovida (Narikyo)',
        ShogiTokin: 'Tokin (Peón Promovido)',
    },
    ca: {
        King: 'Rei',
        Queen: 'Dama',
        Rook: 'Torre',
        Bishop: 'Alfil',
        Knight: 'Cavall',
        Pawn: 'Peó',
        GrantKing: 'Rei',
        Aanca: 'Aanca',
        Unicorn: 'Unicorn',
        Lion: 'Lleó',
        Crocodile: 'Cocodril',
        Giraffe: 'Girafa',
        Grantpawn: 'Peó',
        ChaturajiKing: 'Rei',
        ChaturajiElephant: 'Elefant',
        ChaturajiHorse: 'Cavall',
        ChaturajiBoat: 'Vaixell',
        ChaturajiPawn: 'Peó',
        FourSeasonsKing: 'Rei',
        FourSeasonsGeneral: 'General',
        FourSeasonsRook: 'Torre',
        FourSeasonsKnight: 'Cavall',
        FourSeasonsBishop: 'Alfil',
        FourSeasonsPawn: 'Peó',
        General: 'General',
        Alferza: 'Alferza',
        Alfil: 'Alfil',
        Elephant: 'Elefant',
        Horse: 'Cavall',
        Boat: 'Vaixell',
        XiangqiGeneral: 'General',
        XiangqiAdvisor: 'Conseller',
        XiangqiElephant: 'Elefant',
        XiangqiHorse: 'Cavall',
        XiangqiChariot: 'Carro',
        XiangqiCannon: 'Canó',
        XiangqiSoldier: 'Soldat',
        Khun: 'Senyor (Khun)',
        Met: 'Llavor (Met)',
        Khon: 'Noble (Khon)',
        Ma: 'Cavall (Ma)',
        Ruea: 'Vaixell (Ruea)',
        Bia: 'Cauri (Bia)',
        Biangai: 'Cauri promocionat (Biangai)',
        ShogiKing: 'Rei (Gyoku)',
        ShogiRook: 'Torre (Hisha)',
        ShogiBishop: 'Alfil (Kaku)',
        ShogiGold: 'General d\'Or (Kin)',
        ShogiSilver: 'General de Plata (Gin)',
        ShogiKnight: 'Cavall (Keima)',
        ShogiLance: 'Llancer (Kyosha)',
        ShogiPawn: 'Peó (Fuhyo)',
        ShogiDragon: 'Drac (Ryu)',
        ShogiHorse: 'Cavall Drac (Uma)',
        ShogiPromotedSilver: 'Plata Promocionada (Narigin)',
        ShogiPromotedKnight: 'Cavall Promocionat (Narikei)',
        ShogiPromotedLance: 'Llança Promocionada (Narikyo)',
        ShogiTokin: 'Tokin (Peó Promocionat)',
    },
};

export const useTranslation = () => {
    const language = useGameStore((state) => state.language) || 'en';

    const t = translationsMap[language] || en;

    const getVariantMeta = (variantId: string): VariantMetaI18n => {
        const metas = variantMetaMap[language] || variantMeta_en;
        return metas[variantId] || variantMeta_en[variantId] || {
            title: VariantRegistry.getTitle(variantId),
            origin: '',
            tag: '',
            desc: '',
        };
    };

    const getVariantCodex = (variantId: string): VariantCodexI18n => {
        const codices = variantCodexMap[language] || variantCodex_en;
        return codices[variantId] || variantCodex_en[variantId] || fallbackVariantCodex;
    };

    const getPieceName = (name: string): string => {
        const names = pieceNamesMap[language] || pieceNamesMap.en;
        return names[name] || name;
    };

    return {
        t,
        language,
        getVariantMeta,
        getVariantCodex,
        getPieceName,
    };
};
